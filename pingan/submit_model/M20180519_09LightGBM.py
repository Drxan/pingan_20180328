...
select_features = ['busy_month', 'free_month', 'busy_weekday', 'free_weekday', 'busy_day', 'free_day', 'busy_hour',
                   'free_hour', 'busy_period', 'free_period', 'speed_mean','speed_max', 'speed_std','speed_median',
                   'hour14_speed_mean', 'hour15_speed_mean','period0_speed_mean', 'period1_speed_mean',
                   'period2_speed_mean','period3_speed_mean','period0_speed_std','period1_speed_std',
                   'period2_speed_std','period3_speed_std', 'height_max','height_min','height_std','height_median',
                   'max_lon', 'min_lon', 'max_lat','min_lat', 'lon_ratio', 'lat_ratio', 'dis_start', 'loc_most',
                   'loc_most_freq', 'loc_entropy','loc_num']
select_cat_features = ['busy_month', 'free_month', 'busy_weekday', 'free_weekday', 'busy_day', 'free_day', 'busy_hour',
                   'free_hour', 'busy_period', 'free_period','loc_most']
...
def process(CURRENT_PATH):
    ...
    result_params, train_score, val_score = randomized_search(x_train, x_val, y_train, y_val, lgbr, lgb_params,
                                                              param_dist, new_feature_names, select_cat_features,
                                                              n_iter_search)
    ...
    lgbr.fit(x_train, y_train, eval_metric='rmse', feature_name=new_feature_names,
             categorical_feature=select_cat_features, eval_set=[(x_train, y_train), (x_val, y_val)],
    ...
    lgbr.fit(train_x, targets, eval_metric='rmse', feature_name=new_feature_names,
             categorical_feature=select_cat_features, verbose=0)
