...
def extract_user_features(term):
    ...
    num_of_trips = term['TRIP_ID'].nunique()

    features.extend([record_num, num_of_trips])
    ...
    features.extend([max_lon, min_lon, max_lat, min_lat, lon_ratio, lat_ratio, dis_start])

    return features


feature_names = ['record_num','num_of_trips','busy_period', 'free_period', 'period_mean_num', 'period_std_num',
               ...
               'min_lat','lon_ratio','lat_ratio','dis_start']
cat_features = ['busy_period', 'free_period', 'busy_month', 'free_month', 'busy_day', 'free_day',
                'busy_weekday', 'free_weekday','busy_hour', 'free_hour']
...
def process(CURRENT_PATH):
    ...
    del x_svd
    targets = (np.array(targets) > 0).astype(np.int32)
    ...
    print('train auc:{0}'.format(lgbr.evals_result_['train']['auc'][bst_iteration - 1]))
    ...
    print('[5] Predicting...')
    start = time.time()
    preds = lgbr.predict_proba(test_x)
    preds = preds[:, 1]
