...
import pingan
from pingan.data_helper_mulprocess import get_test_batch_data_list, get_train_batch_data_list
...
def process(CURRENT_PATH):
    ...
    for i in range(KFOLD):
        model = models.create_cnn2((GROUP_SIZE, x_dim),(x_dim,), GROUP_SIZE)
        print(model.summary())
        ...
        train_hist = model.fit_generator(
            get_train_batch_data_list(x_train, y_train, batch_size=BATCH_SIZE, group_size=GROUP_SIZE),
            steps_per_epoch=train_steps,
            epochs=EPOCHES,
            callbacks=[early_stop, check_point, reduce_lr],
            validation_data=get_train_batch_data_list(x_val, y_val, batch_size=2*BATCH_SIZE, group_size=GROUP_SIZE),
    ...
    for model in cv_models:
        cv_pred = model.predict_generator(get_test_batch_data_list(test_x, len(test_x) // GROUP_SIZE, GROUP_SIZE), steps=1)
